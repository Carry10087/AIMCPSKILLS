...
# 模式1: 简单拼接
sign = hashlib.md5(f"{timestamp}{page}{salt}".encode()).hexdigest()

# 模式2: 有序拼接（注意参数顺序）
sign_str = f"clientIp={ip}&checkId={cid}&uuid={uid}&track={track}"
sign = hashlib.md5(sign_str.encode()).hexdigest()

# 模式3: 字节salt
salt = b"\xa3\xac\xa1\xa3fdjf,jkgfkl"  # 非UTF8的salt
...
**Step 1：确认AES参数（从浏览器实证）**
```javascript
// MCP: js-reverse-mcp → evaluate_script
const origEncrypt = CryptoJS.AES.encrypt;
CryptoJS.AES.encrypt = function(msg, key, cfg) {
    console.log('=== AES ENCRYPT ===');
    console.log('msg:', msg);                           // 明文
    console.log('key:', key.toString());                 // 密钥
    console.log('cfg:', JSON.stringify(cfg));            // {mode, iv, padding}
    const result = origEncrypt.call(this, msg, key, cfg);
    console.log('result:', result.toString());           // Base64密文
    console.log('ciphertext hex:', result.ciphertext.toString());
    return result;
};
```
...
if pad_len == 16:
    pad_len = 0
padded = data.encode() + b'\x00' * pad_len
ciphertext = cipher.encrypt(padded)
result = base64.b64encode(ciphertext).decode()

# === 情况3: ECB + PKCS7 ===
cipher = AES.new(key, AES.MODE_ECB)  # ECB不需要IV
padded = pad(data.encode(), AES.block_size)
ciphertext = cipher.encrypt(padded)
result = base64.b64encode(ciphertext).decode()
```
...
# 方法2: 从接口动态获取
import requests
resp = requests.post("https://xxx.com/api/getPublicKey")
pub_key_pem = resp.json()["data"]["publicKey"]
public_key = RSA.import_key(pub_key_pem)

# 加密
cipher = PKCS1_v1_5.new(public_key)
encrypted = cipher.encrypt(aes_key_bytes)  # 通常加密AES密钥
...
```javascript
// 搜素编码函数附近的大数组
// 特征：64个字符的数组，包含 A-Z, a-z, 0-9, +, /

// 小红书 lookup 示例：
var lookup = ["Z","m","s","e","r","b","B","o","H","Q","t","N","P","+","w","O","c","z",
              "a","/","L","p","n","g","G","8","y","J","q","4","2","K","W","Y","j","0",
              "D","S","f","d","i","k","x","3","V","T","1","6","I","l","U","A","F","M",
              "9","7","h","E","C","v","u","R","X","5"];
```
...
def custom_b64_encode(data):
    ...
    trans_back = str.maketrans(STANDARD, CUSTOM)
    return std.translate(trans_back)

# 方案B: 直接实现（更安全，不依赖base64库）
def triplet_to_base64(e, lookup):
    return lookup[e >> 18 & 63] + lookup[e >> 12 & 63] + lookup[63 & e >> 6] + lookup[63 & e]

def b64_encode(data, lookup):
    """完整的自定义Base64编码"""
    result = []
    for i in range(0, len(data), 3):
        if i + 2 < len(data):
            val = (data[i] << 16) + (data[i+1] << 8) + data[i+2]
            result.append(triplet_to_base64(val, lookup))
        else:
            # 处理剩余1-2字节
            remaining = data[i:]
            if len(remaining) == 1:
                result.append(lookup[remaining[0] >> 2] + lookup[(remaining[0] & 3) << 4] + "==")
            else:
                val = (remaining[0] << 8) + remaining[1]
                result.append(lookup[val >> 10] + lookup[(val >> 4) & 63] + lookup[(val & 15) << 2] + "=")
    return "".join(result)
```
...
```python
# 用浏览器捕获的样本测试
browser_output = "ZmserbBo..."  # 浏览器生成的
local_output = custom_b64_encode(data_bytes, CUSTOM)
assert browser_output == local_output, "编码结果不一致!"
```
...
class JDH5STSigner:
    ...
    核心流程：
    1. 生成指纹 fp（16位随机hex，首次生成后缓存）
    2. 请求 cactus.jd.com/request_algo → 获取 tk + algo 函数
    3. 用 algo 函数计算 t（基于请求参数）
    4. MD5(t + query_string + t) → p5
    5. AES 加密环境参数 → p8
    6. 拼接完整 h5st
    """
    
    def __init__(self, app_id="76ef7", version="4.7"):
        self.app_id = app_id
    ...
    def request_algo(self):
        ...
        Response: {
            "data": {
                "tk": "s01_xxxxxxxxxxxxx",
                "fp": "a1b2c3d4e5f6a7b8",
                "algo": "function algo(t, r, i) { ... }",
                "ts": 1733123456789
            }
        }
        """
        ...
        payload = {
            ...
            "platform": "web",
            "expandParams": self._build_expand_params(),
        }
        resp = self.session.post(url, json=payload)
        data = resp.json()
        if data.get("status") == 200:
            algo_data = data["data"]
            self.tk = algo_data["tk"]
            self.algo_params = algo_data
            return algo_data
        raise Exception(f"request_algo failed: {data}")
    
    def _build_expand_params(self):
        """构建 expandParams（AES加密的环境信息）
        ...
        # AES加密（密钥通常由 cactus 返回或内置）
        return self._aes_encrypt_env(json.dumps(env, separators=(',', ':')))
    
    def _aes_encrypt_env(self, plaintext):
        """AES-CBC 加密环境参数
        
        密钥和IV通常从 algo 函数中提取或固定内置
        此处使用通用模式，实际需从JS中确认
        """
        # 实际密钥需要从JS源码中提取
        key = b"0123456789abcdef"  # 示例，需要从JS确认
        iv = key
        cipher = AES.new(key, AES.MODE_CBC, iv)
    ...
    def _compute_t(self, algo_func_str, query_params):
        """执行 algo 函数计算 t 值
        
        algo 函数通常格式：function algo(t, r, i) { ... }
        - t: 请求参数（query string 或 body）
        - r: 随机数/token
        - i: 额外参数
        
        方案A: Node.js 子进程执行（推荐，保证一致性）
        方案B: Python 翻译 algo 逻辑（维护成本高）
        """
        import subprocess
        ...
        const result = algo(
            "{query_params}",
            "{self.algo_params.get('tk', '')}",
            "{self.algo_params.get('fp', '')}"
        );
        ...
        return json.loads(result.stdout)["t"]
    
    def sign(self, query_string="", body_json=None):
        """生成完整 h5st 签名
        
        Args:
            query_string: URL查询字符串（不含?）
            body_json: POST请求体(dict)
        Returns:
            h5st 签名字符串
        """
        # 确保有 tk 和 algo
        ...
        # 计算 p5（MD5签名）
        if body_json:
            query_str = json.dumps(body_json, separators=(',', ':'))
        else:
            query_str = query_string
        
        t = self._compute_t(
            self.algo_params.get("algo", ""), query_str
        )
        sign_raw = f"{t}{query_str}{t}"
        p5 = hashlib.md5(sign_raw.encode()).hexdigest()
        
        p6 = self.version
        p7 = str(int(time.time() * 1000))
        p8 = self._build_expand_params()
        
        h5st = ";".join([p1, p2, p3, p4, p5, p6, p7, p8])
        return h5st


# === 使用示例 ===
if __name__ == "__main__":
    signer = JDH5STSigner(app_id="76ef7")
    
    # GET 请求签名
    params = "keyword=手机&page=1&psort=3"
    h5st = signer.sign(query_string=params)
    print(f"h5st: {h5st}")
    ...
    resp = requests.get(
        f"https://api.m.jd.com/api?{params}",
        headers={**headers, "h5st": h5st}
    )
    print(resp.json())
```
...
```python
# douyin_sign.py — 抖音 a_bogus / X-Bogus 签名生成器
import time
import random
import hashlib
import json
...
class DouyinSigner:
    ...
    def _modified_rc4(self, key: bytes, data: bytes) -> bytes:
        """魔改 RC4 加密（抖音对标准 RC4 做了修改）
        
        标准 RC4: S盒256字节，KSA+PRGA
        抖音魔改：S盒初始化顺序调整，PRGA 增加额外混淆步骤
        """
        S = list(range(256))
        j = 0
        key_len = len(key)
        ...
        # KSA（抖音魔改版：增加了额外的扰动）
        for i in range(256):
            j = (j + S[i] + key[i % key_len] + i) % 256  # 标准RC4没有+i
            S[i], S[j] = S[j], S[i]
        
        # 二次混淆（抖音特有）
        for i in range(256):
            j = (j + S[i] + key[(i * 3) % key_len]) % 256
            S[i], S[j] = S[j], S[i]
    ...
    def _sm3_hash(self, data: bytes) -> str:
        ...
        return sm3_hash(data.hex().encode())
    
    def _build_env_string(self, url_params: str, post_data: str = "") -> str:
        """构建环境参数字符串（输入到加密函数）
        ...
        # 拼接格式（实际需从JS确认）
        env = (
            f"user_agent={self.ua}|"
            f"timestamp={timestamp}|"
            f"random={random_num}|"
            f"url_params={url_params}"
        )
        if post_data:
            env += f"|post_data={post_data}"
        
        return env
    
    def get_a_bogus(self, url_params: str, post_data: str = "") -> str:
        """生成 a_bogus 签名
        
        Args:
            url_params: URL查询字符串（不含?）
            post_data: POST请求体（GET请求传空字符串）
        Returns:
            a_bogus 签名字符串
        """
        import subprocess
        
        # 方案A：Node.js 子进程调用签名（推荐，保证与浏览器一致）
        # 需要先准备好签名JS文件
        js_script = f"""
        const sign = require('./douyin_sign_wasm.js');
        const result = sign.generate({{
            url: "{url_params}",
            data: "{post_data}",
            ua: "{self.ua}"
        }});
    ...
    def get_x_bogus(self, url_params: str, post_data: str = "") -> str:
        """生成 X-Bogus 签名（Header 版本）"""
        # X-Bogus 与 a_bogus 算法核心相同
        return self.get_a_bogus(url_params, post_data)


# === 使用示例（在线API方案） ===
# 如果不想自己维护签名算法，可以使用在线签名服务

def get_a_bogus_online(url_params: str, ua: str, data: str = "") -> str:
    """使用在线 API 获取 a_bogus
    
    开源项目: jackluson/a_bogus_douyin
    提供免费的 Cloudflare Worker 签名服务
    """
    resp = requests.post(
        "https://abogus.jackluson.workers.dev/",
        json={
            "url": url_params,
            "ua": ua,
            "data": data,
...
if __name__ == "__main__":
    ...
    # 抖音搜索接口
    params = "keyword=美食&offset=0&count=10"
    a_bogus = get_a_bogus_online(params, ua)
    
    url = f"https://www.douyin.com/aweme/v1/web/search/item/?{params}&a_bogus={a_bogus}"
